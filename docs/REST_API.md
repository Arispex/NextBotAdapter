...
### GET `/nextbot/users/{user}/map-image`

实时生成指定玩家视角下的地图 PNG 图片并以 Base64 返回。仅渲染该玩家"已探索"区域的真实瓦片色，其余区域为不透明纯黑。

**权限：** `nextbot.users.map_image`
...
所谓"已探索"由本插件根据玩家在线时的位置采样自行累积，**不等同于客户端 `<player>.map` 文件中的真实 fog-of-war**：
