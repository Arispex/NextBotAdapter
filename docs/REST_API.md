...
| 名称     | 位置     | 必填 | 说明                                                                                                |
|----------|----------|------|-----------------------------------------------------------------------------------------------------|
| `player` | 查询参数 | 否   | 指定玩家账号名时，仅渲染该玩家"已探索"区域的真实瓦片色，其余区域为不透明纯黑。不传时返回完整地图。 |

**模拟探索说明**

带 `player` 参数时，所谓"已探索"由本插件根据玩家在线时的位置采样自行累积，**不等同于客户端 `<player>.map` 文件中的真实 fog-of-war**：

- 仅记录插件启用之后玩家走过的区域。每次采样以玩家所在 tile 为中心 stamp 一个 141×87 瓦片的矩形（按 1080p 客户端默认视野近似）；连续两次采样之间自动沿线段插值，高速飞行 / 坐骑下也能填补路径。
- 单次采样间距超过 500 瓦片时视为瞬移（魔镜、海螺、回程药水、地狱井等），只标记终点周围而不沿途填充。
- 不会捕获仅"远远看到"但未靠近的区域。
...
| 状态码 | `error`               | 原因                                       |
|--------|-----------------------|--------------------------------------------|
| 400    | `User was not found.` | 带 `player` 时找不到对应的 TShock 注册账号 |
| 500    | `<异常信息>`          | 地图生成失败                               |
