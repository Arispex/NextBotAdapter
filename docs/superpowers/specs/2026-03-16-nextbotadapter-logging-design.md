...
- 默认采用 `动作 + 对象 + 结果 + 关键上下文`；无关键上下文时可省略
- 成功：`动作 + 对象 + 结果`
- 失败：`动作 + 对象 + 结果，原因：...`
- 拒绝 / 回退 / 降级：`动作 + 对象 + 结果，补充自然语言上下文`
- 避免空洞表达，例如“进入方法”“处理完成”
- 避免大段 `key=value` 风格堆砌
- 避免整对象、整请求、整响应的原样输出
...
- `加载白名单配置成功。`
- `加载白名单配置失败，将回退为默认配置，原因：...`
- `保存白名单数据成功，当前共有 3 个条目。`
- `拒绝玩家 Alice 进入服务器，原因：You are not on the whitelist.`
- `初始化插件完成，已注册 4 个 REST 端点。`
...
- `创建默认白名单配置文件成功。`
- `加载白名单配置成功。当前启用状态为 True，区分大小写为 True。`
- `加载白名单配置失败，将回退为默认配置，原因：...`
- `保存白名单配置成功。当前启用状态为 True，区分大小写为 True。`
- `创建默认白名单文件成功。`
- `加载白名单数据成功，当前共有 3 个条目。`
- `加载白名单数据失败，将回退为空白名单，原因：...`
- `保存白名单数据成功，当前共有 3 个条目。`
...
- `添加玩家 Alice 到白名单成功。`
- `添加玩家 Alice 到白名单失败，原因：...`
- `将玩家 Alice 移出白名单成功。`
- `将玩家 Alice 移出白名单失败，原因：...`
- `重新加载白名单状态成功。当前启用状态为 True，区分大小写为 True，当前共有 3 个条目。`
...
- `注册 REST 端点完成，共 4 个。`
- `注册玩家信息校验钩子完成。`
- `应用当前白名单配置完成。启用状态为 True，区分大小写为 True，当前共有 3 个条目。`
- `检测到白名单功能未启用，玩家进入服务器时将不会进行白名单校验。`
- `开始释放插件资源。`
- `拒绝玩家 Alice 进入服务器，原因：You are not on the whitelist.`
