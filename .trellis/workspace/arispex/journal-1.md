...
## Session 20: 安全加固、caseSensitive 移除、NextBot 同步功能

**Date**: 2026-04-12
**Task**: 安全加固、caseSensitive 移除、NextBot 同步功能
...
| 功能 | 说明 |
|------|------|
| 安全加固 | 白名单添加接口拒绝已在黑名单中的用户；黑名单未启用时启动日志提示 |
| 黑白名单联动 | 加入黑名单自动移除白名单并踢出在线玩家；移除黑名单自动加入白名单 |
| 校验顺序 | 入服校验改为黑名单优先于白名单 |
| caseSensitive 移除 | 删除白名单和黑名单的 caseSensitive 配置项，统一使用 OrdinalIgnoreCase，消除跨名单联动时的大小写不一致问题 |
| NextBot 同步 | 启动时从 NextBot API 获取用户列表，非封禁用户同步到白名单，封禁用户同步到黑名单（含原因），本地多余条目自动移除。新增 sync 配置段控制 |

**新增文件**: SyncSettings.cs, NextBotUserResponse.cs, NextBotSyncService.cs, NextBotSyncServiceTests.cs
**修改文件**: NextBotAdapterPlugin.cs, WhitelistSettings.cs, WhitelistService.cs, WhitelistEndpoints.cs, BlacklistEndpoints.cs, NextBotAdapterConfig.cs, PluginConfigService.cs, NextBotSessionProbeService.cs, ConfigEndpointsTests.cs, PluginConfigServiceTests.cs, WhitelistServiceTests.cs, WhitelistEndpointsTests.cs, BlacklistServiceTests.cs, NextBotSessionProbeServiceTests.cs, CONFIGURATION.md, REST_API.md
...
## Session 21: 黑名单联动 TShock 游戏内封禁 + autoLogin 封禁检查修复

**Date**: 2026-04-20
**Task**: 黑名单联动 TShock 游戏内封禁 + autoLogin 封禁检查修复
...
| 项目 | 说明 |
|------|------|
| feat(blacklist) | 黑名单 add / remove 时自动联动 TShock 游戏内封禁：若用户名对应已注册 TShock 账号，调用 `TShock.Bans.InsertBan`（acc: 前缀，banningUser=NextBotAdapter）执行永久封禁；remove 时通过 `RetrieveBansByIdentifier + RemoveBan` 对称解除。REST 接口与 NextBot 启动同步均接入。未注册账号静默跳过。 |
| fix(security) | autoLogin 绕过 TShock 封禁检查的漏洞：原 `PerformAutoLogin` 直接设置 `player.Account` + `IsLoggedIn` 后返回，不触发 `PlayerHooks.OnPlayerPostLogin`，导致 TShock 订阅该事件的 `Bans.CheckBan`（检查 name/ip/uuid/acc 四种 identifier）完全跳过。修复：在 `PerformAutoLogin` 末尾触发 `PlayerHooks.OnPlayerPostLogin(player)`，与手动 `/login` 命令走同一条 hook 链；移除冗余的 `StartSession` 直接调用。 |

## 关键反编译发现

`/login`（AttemptLogin）内部**不**直接查封禁，而是在末尾调用 `PlayerHooks.OnPlayerPostLogin`；TShock 订阅该事件的 `OnPlayerLogin` 里调用 `Bans.CheckBan(player)`，后者构造 `[name:, ip:, uuid:, acc:]` 四种 identifier 列表遍历 `TShock.Bans.Bans`，命中则 `player.Disconnect` 并输出标准格式的 ban 消息（含 TicketNumber、永久/剩余时间分支）。

## 关键设计决策

- 新建 `ITShockUserBanService` 接口 + `TShockUserBanService` 实现，消费侧通过接口注入，测试用 `FakeTShockUserBanService`。实现本身不单元测试（触 `TShock` 静态单例），沿用 `BlacklistEndpoints.KickOnlinePlayer` 的既有模式。
- 中间曾在接口加 `IsAccountBanned(out string)`，后发现触发 `PlayerPostLogin` hook 更能和 `/login` 完全对齐，遂撤回这个方法，保持接口纯粹（只暴露 Ban / Unban）。
- 游戏内 ban 失败不回滚黑名单写入：黑名单是 source of truth，游戏内 ban 是尽力而为的副作用，失败仅记日志。

## Updated Files

- `NextBotAdapter/Services/Security/TShockUserBanService.cs`（新）
- `NextBotAdapter/Rest/BlacklistEndpoints.cs`：`BanService` 静态属性 + Add/Remove 钩子
- `NextBotAdapter/Services/NextBot/NextBotSyncService.cs`：构造器注入 + SyncBlacklist 钩子
- `NextBotAdapter/Plugin/NextBotAdapterPlugin.cs`：注入 banService；`PerformAutoLogin` 末尾触发 `PlayerHooks.OnPlayerPostLogin`，移除冗余 StartSession
- `NextBotAdapter.Tests/FakeTShockUserBanService.cs`（新）
- `NextBotAdapter.Tests/BlacklistEndpointsTests.cs`、`NextBotSyncServiceTests.cs`：新增 ban 钩子断言（共 +7 测试）
- `docs/CONFIGURATION.md`：黑名单 TShock 联动 + autoLogin 封禁检查说明
- `docs/REST_API.md`：blacklist add/remove 接口副作用说明

## 验证与发布

- `dotnet build -c Release` 0 warning 0 error
- `dotnet test` 225/225 通过（原 218 + 新增 7）
- tag `v1.1.1` 打在 `0e284a2`，推送至 origin
- GitHub release v1.1.1 已发布并附 `NextBotAdapter.dll`（2.13 MB Release 构建）

## 已知未处理项

- 代码里 `Version` 仍为 `1.1.0`（未跟 tag 一起 bump），DLL 元数据 / `/nb help` 显示会与 tag 不一致，用户已知晓暂未处理
